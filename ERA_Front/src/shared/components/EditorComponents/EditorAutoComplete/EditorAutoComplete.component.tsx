...
const Dropdown = ({label, initValue, required, inputData, outputData}: IDropdownProps) => {
  ...
  const onChange = useCallback((e: string) => {
    ...
    setText(value);
    if (value) {
        setData(inputData.filter(item => item.key!==null? item.key.toString().toLowerCase().includes(value.toLowerCase()): '0'));
    } else {
      setData(inputData);
    }
  }, []);
