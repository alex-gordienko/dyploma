...
import { FieldName, PhotoBlock, Photo, StyledEditor, EditorForm } from './EditorBlock.styled';
...
const EditorBlock = forwardRef(({
  ...
  const initUser: IFullDataUser = existUserData? existUserData: initialState.user;
  ...
  const outputContryData = (res:string)=>{
    ...
    var newBuf = res!==""?
      cities.filter(
        element=> element.country_id===contries.find(
          element=> element.name_en===res)!.id)
      ...
      <StyledEditor>
        ...
        <EditorForm>
          ...
          <AutoComplete
              ...
              inputData={Array.from(contries, element=> <div key={element.name_en}>{element.name_en}</div>)}
      ...
      />
          {visible? 
              <AutoComplete
                  ...
                  inputData={citiesBuf.map((city=> <div key={city}>{city}</div>))}
                  outputData={outputCityData}
                />
              :null
          }
          <DateItem label="Birthday:" 
            disabled={isDisabled}
            required={true}
            placeholder="Choose date"
            value={user.Birthday} 
            onChange={(e)=> setUser({...user,Birthday: e})}
            onInputError={user.Birthday==="mm/dd/yyyy"? "Incorrect date": undefined}
          />
          <TextInputItem
            required={true}
            label="E-mail:"
