...
const GoogleMapComponent = (action:IGoogleMapProps) => {
    const [marker, newMarker] = useState(action.mode==="Posts"?{ lat: action.aloneMarker? action.aloneMarker.position.lat:0, lng: action.aloneMarker? action.aloneMarker.position.lng:0 }: null);
    ...
    const getPosition = (e: any) => {
      ...
      if (action.newPosition) action.newPosition(e.latLng.lat(), e.latLng.lng());
      }
    };
  if(action.mode==="Posts"){
    return action.data ? (
    ...
    : action.aloneMarker? 
      (
        <GoogleMap
        defaultZoom={10}
        defaultCenter={{ lat: action.aloneMarker.position.lat, lng: action.aloneMarker.position.lng }}
        defaultOptions={{ styles: mapStyle }}
        onClick={getPosition}
      >
        <Marker
