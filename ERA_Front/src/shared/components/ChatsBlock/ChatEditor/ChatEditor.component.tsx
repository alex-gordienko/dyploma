...
import { Avatar, PhotoBlock, Photo, StyledEditor, EditorForm } from './ChatEditor.styled';
import { IChat, IMember, IPreviewChat, IFullDataUser } from '../../../../App.types';
import Jimp from "jimp";
import { ButtonBlock, FieldName, Element } from "../../EditorComponents/EditorComponents.styled";
  ...
  return (
      <StyledEditor>
        ...
        <EditorForm>
          ...
          <ChipsDropdown
            label='Add members'
            restriction={chat.type==='private'?{type: 'only one element'}: undefined}
            inputData={peoples.filter(member=> member.idUsers!==currentUser.idUsers).map(member=>{
                return <div key={member.username} style={{display: 'inline-flex'}}>
                    <Avatar>
                    <img key={member.idUsers} src={member.avatar!== null? member.avatar : defaultAvatar}/>
                    </Avatar>
                    {member.username}
                </div>
            })}
            alreadySelected={existChatData? 
