...
import { IFullDataUser, IPreviewChat, IChat, IMember, ISearchedUser } from "../../../App.types";
import { chatServerAdress, httpPost } from "../../../backend/httpGet";
...
const ChatsFeed = ({ currentUser, onError}: IChatsFeedProps) => {
  ...
  const socket: SocketIOClient.Socket = io.connect(chatServerAdress);
  ...
  const saveChanges = (command: "create chat"|"edit chat"|'delete chat', chat:IPreviewChat)=>{
    ...
    if(!newChat.members.find((member: IMember)=> member.idUsers===currentMember.idUsers)) newChat.members.push(currentMember);
    ...
    console.log(data);
        if(data.chatlist) {
          ...
          setIsTyping(prevState=>{
            let newChats = prevState.concat({room: data.chatlist.chatID.toString(), typing:['']});
            return newChats
          });
        }
    setShowModal(false);
  ...
  const onDeleteMessage = (data:any)=>{
    ...
    setRoom(prevState=>{
      return{
        ...
        type: prevState.type,
        members: prevState.members,
        messages: prevState.messages.filter(msg=> !data.find((elem:any)=> elem===msg.id))
      }
    })
  }
  ...
  const selectChatRoom = (chatroom: string) =>{
    setRoom(prevState=>{
      return{
        ...
        avatar: chats.filter(chat=>chat.chatID.toString()===chatroom)[0].avatar?
                chats.filter(chat=>chat.chatID.toString()===chatroom)[0].avatar
                :defaultAvatar,
        type: chats.filter(chat=>chat.chatID.toString()===chatroom)[0].type,
        members: chats.filter(chat=>chat.chatID.toString()===chatroom)[0].members,
        messages: []
      }
    })
    if(socket.emit("join room", {'user':currentUser.username, 'chatroom': chatroom})){
    }
    else onError("Chat server error, Please, try again");
  ...
  const onDelete = (chatroom: string, justHideFromAuthor: boolean, selectedMessageIDs: number[]) =>{
    console.log('Chat '+chatroom+', command: delete, data: '+selectedMessageIDs+' just hide: '+justHideFromAuthor);
    setRoom(prevState=>{
      return{
        chatID: prevState.chatID,
        name: prevState.name,
        ...
        type: prevState.type,
        members: prevState.members,
        messages: prevState.messages.filter(message=> !selectedMessageIDs.find(selId=> selId===message.id))
      }
    })
    socket.emit('delete', {'room':chatroom, 'user':currentUser.username, 'justHide': justHideFromAuthor,'data': selectedMessageIDs})
  }
      ...
      <Container>
          ...
          <StyledChatsFeed>
            <Chats
              ...
              onEdit={(e)=> {setSelectedChatToEdit(e); searchPeople()}}
          ...
          >
            <ChatEditor
              ref={childRef}
              currentUser={currentUser}
              existChatData={selectedChatToEdit.chatID!=='0'?
                selectedChatToEdit
                :undefined}
              peoples={members}
              onMembersInputChange={(e)=>searchPeople({username: e, country:'',city:'', date:''})}
              isDisabledToApply={(e)=>setIsDisabled(e)}
              saveUserChanges={(e)=>
                saveChanges(selectedChatToEdit.chatID!=='0'?
                'edit chat':'create chat',e)}
            />
          </Modal>
      </Container>
  );
