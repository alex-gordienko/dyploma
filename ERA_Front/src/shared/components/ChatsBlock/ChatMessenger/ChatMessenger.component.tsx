...
import StyledMessenger, 
      { Avatar,
        ...
        MessengerFooterBlock } from "./ChatMessenger.styled";
...
interface IChatProps {
  ...
  onDelete:(chatroom: string, justHideFromAuthor: boolean, selectedMessageIDs: number[]) => void;
}

const Chat = ({ currentUser, isTyping, data, onTyping, onSend, onDelete }: IChatProps) => {
  ...
  const MessageAuthorBlock = (item:IMessage, indx: number)=>{
    let avatar = data.members.find(member=> member.idUsers===item.id_author)!.avatar!==null?
    ...
    return data.type==='public'? 
      indx>=1 && item.id_author!==data.messages[indx-1].id_author?
        <MessageAuthor Rating={data.members.find(member=> member.idUsers===item.id_author)!.rating}>
          <img className='avatar' src={avatar}/>
          {data.members.find(member=> member.idUsers===item.id_author)?.username}
        </MessageAuthor>
      : indx<1?
          <MessageAuthor Rating={data.members.find(member=> member.idUsers===item.id_author)!.rating}>
            <img className='avatar' src={avatar}/>
  ...
  const MessageDateBlock = (item:IMessage, indx: number) =>{
    ...
    var prevMessageDate = new Date(indx===0? data.messages[indx].time: data.messages[indx-1].time);
    var title;
    ...
    if(indx===0) return <MessageDate title={title}>{title}</MessageDate>
      ...
      if(birth.getDate()!==prevMessageDate.getDate()){
        if(today.getDate()-birth.getDate()===0) return <MessageDate title={title}>Today</MessageDate>
        else if(today.getDate()-birth.getDate()===1) return <MessageDate title={title}>Yesterday</MessageDate>
        else if(today.getDate()-birth.getDate()>2) return <MessageDate title={title}>{title}</MessageDate>
      }
      else return null
    }
  }
  ...
  const MessageTimeBlock = (itemTime: string)=>{
    ...
    return <MessageTime>
              {hour.toString()}:{minutes<10? '0'+minutes.toString():minutes.toString()}
          </MessageTime> 
  ...
  const selectMessage = (messageID: number)=>{    
      setSelectedMessages(prevState=>{
        if(prevState.find(searched=> searched.id===messageID)===undefined){
          ...
          let newSelected:IMessage[]= prevState.concat(data.messages.filter(searched=> searched.id===messageID));
          return newSelected
        }
        else{
          console.log('Already selected');
            let newSelected:IMessage[]= prevState.filter(searched=>searched.id!==messageID)
            return newSelected
        }
  ...
  return data.chatID!=='0'?(
    <StyledMessenger>
          ...
          <Chatname>
            ...
            <div>
              {isTyping.map(roomTypers=>{
                if(roomTypers.room===data.chatID && roomTypers.typing.length>1) return roomTypers.typing.filter(typer=>typer!=='')+' is typing...'
              })}
          </div>
          </Chatname>
      </MessangerHeader>
      :<MessangerHeader>
          <ButtonBlock>
            <button className='label-drop' onClick={dropSelection}>
              <Cross fill='black' width='14' height='14'/>{selectedMessages.length} selected
            </button>
      ...
      <MessengerContentBlock>
        {data.messages!==[]?
          data.messages
            .filter(msg=> !(msg.isHiddenFromAuthor && msg.id_author===currentUser.idUsers))
            .map((item, indx) => {           
              return (
                <div>
                  {MessageDateBlock(item,indx)}
                  <MessageContainer
                    myMessage={item.id_author===currentUser.idUsers? true: false}
      ...
      <MessengerFooterBlock>
        <Modal
          ...
          name='Delete'
          onOK={()=>{
            onDelete(data.chatID, justHide, selectedMessages.map(sel=> sel.id));
            setShowModal(false);
            setSelectedMessages([]);
          }}
        ...
        >
          <div>
            ...
            <input type="checkbox" onChange={(e)=>{setJustHide(!e.currentTarget.value)}}/>
            Also delete for {data.type==='private'? data.members.find(member=> member.idUsers!==currentUser.idUsers)?.username : 'all'}?
          </div>
        </Modal>
        <TextArea
          onKeyDown={onKeyDown}
