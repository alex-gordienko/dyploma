    ...
    <StyledChatPreview 
      ...
      <CenterBlock>
        ...
        {item.lastMessage.id!==0?
          <Footer>
            {item.members.find(member=> member.idUsers===item.lastMessage.id_author)?.username}: {item.lastMessage.message}
          </Footer>
          :<Footer>
            No Messages here
          </Footer>
      ...
      <RightBlock onMouseLeave={()=>{setVisible(false)}}>
        <MoreIcon style={{width:'15px', height:'15px'}} onMouseEnter={()=>{setVisible(true)}}/>
        <DropDown
          visible={visible}
          onEdit={onEdit}
          onDelete={onDelete}
