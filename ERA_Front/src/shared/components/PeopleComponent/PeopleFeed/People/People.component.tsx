...
const People = ({ item, onClick }: IPeopleProps) => {
  ...
  const getAge = (dob: string) =>{
    ...
    var dobInThisYear = new Date(today.getFullYear(), birth.getMonth(), birth.getDate());
    var Age, title;
  ...
  return (
    <StyledPeople 
      title={item.isBanned? "This user is banned": !item.isConfirm? "This user still not confirm his/her account": undefined}
      ...
      <CenterBlock>
        ...
        <Online>{item.isOnline? "Online": "Last online: "+item.lastOnline}</Online>
        <Body>{item.FirstName} {item.Country!==""? item.Country: null} {item.City!==""? item.City: null} {item.Birthday? getAge(item.Birthday): null}</Body>
      </CenterBlock>
      <RightBlock>
