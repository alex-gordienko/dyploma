...
interface IFilterItemProps {
  ...
  letSearch: (filterName: "Username"|"Date"|"Country"|"City", value: string) => void;
  filters:{
    username: string;
    country:string;
    city:string;
...
*/
const FilterItem = ({contries, cities,  filters, letSearch}:IFilterItemProps) => {
  var date= new Date();
  ...
  return (
    <div>
      ...
      <EditorDate
              ...
              label="Birthday"
              value={date.getFullYear()+"-"+month+"-"+date.getDate()}
              onChange={(e:string)=>{letSearch("Date", e);}}
              />
      <AutoComplete
              key={3}
              label="Country"
