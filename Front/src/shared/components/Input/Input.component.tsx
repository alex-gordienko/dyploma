...
export type IRestrict =
  | IEditorLengthRestriction
  | IEditorDataTypeRestriction
  | IEditorSameRestriction
  | IEditorVariablesRestriction;
...
export default function Input({
  ...
  const onChange = (e: React.FormEvent<HTMLInputElement>) => {
    if (type === "tel") {
      let number = e.currentTarget.value.replace(
    ...
    <StyledInput>
      ...
      <div className="field">
        ...
        {restriction ? (
          <ErrorOnInput>
            {text &&
            restriction.type === "min length" &&
            text.toString().length < restriction.length
              ? "Must be more than " + restriction.length + " characters"
              : null}
            {restriction.type === "type" && typeof text !== restriction.restrict
              ? "Input data must be " + restriction.restrict
              : null}
            {text && restriction.type === "same" && text !== restriction.with
              ? "Fields must be same"
              : null}
            {restriction.type === "only one element"
              ? "Available only one select"
              : null}
          </ErrorOnInput>
        ) : null}
      </div>
